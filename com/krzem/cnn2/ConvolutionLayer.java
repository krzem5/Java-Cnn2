	...
	public CNNVariable err(CNNVariable v,double lr){
		...
		for (int i=0;i<this.lI.length;i++){
			...
			for (int j=0;j<e[i].length;j++){
				double[] of=Activation.applyD(e[i][j],this.af);
				for (int k=0;k<e[i][0].length;k++){
					s+=of[k]*e[i][j][k]*lr;
					if (Double.isNaN(s)){
						System.out.println(v.get_3d()[i][j][k]);
						System.out.printf("%f %f %f %f %d %d %d %s\n",s,of[k],e[i][j][k],lr,i,j,k,this.af);
						System.exit(1);
					}
				}
			}
			// System.out.printf("%s >>> %f\n",this._data(),this.bL[i]+s/(e[i].length*e[i][0].length));
			this.bL[i]+=s/(e[i].length*e[i][0].length);
		}
		v.set(o);
		return v;
	}
