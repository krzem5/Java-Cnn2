...
public class CNN{
	...
	public void train(int maxE,int bs,int tbs,Dataset dt,Dataset test,boolean log){
		if (log==true){
			System.out.printf("Start accuracy: %.2f%%\n",this._test_batch_acc(test,tbs));
		}
		for (int e=0;e<maxE;e++){
			this._train(dt,bs,log);
			if (log==true){
				double ba=this._test_batch_acc(test,tbs);
				int w=18+String.format("%d%d%.2f",e+1,maxE,ba).length();
				String b="";
				for (int i=0;i<w;i++){
					b+="=";
				}
				System.out.printf("%s\nEpoch %d/%d complete (test_batch_acc=%.2f%%)\n%s\n",b,e+1,maxE,ba,b);
			}
		}
	}
	...
	public double acc(Dataset dt){
		...
		for (int i=0;i<dt.size();i++){
			if (p<(int)((double)i/dt.size()*10000)){
				p=(int)((double)i/dt.size()*10000);
				...
				System.out.printf("%.2f%% complete... (t=%.1fs, dt=%.1fs, ETA=%ds)\n",(double)p/100,sd*1e-9,d*1e-9,(int)(sd*1e-9/((double)p*1e-4)-sd*1e-9));
				st=System.nanoTime();
			}
	...
	private double[] _predict(double[][][] i){
		...
		for (Layer l:this.layers){
			...
			v.set(l.out(v));
			if (v._NaN()){
				System.err.printf("NaN value found! (layer=%d)\n",j);
				System.exit(1);
			}
			j++;
		}
	...
	private void _train(Dataset dt,int bs,boolean log){
		...
		long st=-1;
		for (int i=0;i<dt.size();i++){
			if (log==true&&p<(int)((double)i/dt.size()*10000)){
				p=(int)((double)i/dt.size()*10000);
				double sd=(double)(System.nanoTime()-ost);
				double d=(double)(System.nanoTime()-st);
				if (ost==-1||st==-1){
